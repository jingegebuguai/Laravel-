...
@section('content')
	...
	<div class="result_wrap">
        ...
        <div class="result_content">
            <div class="short_wrap">
                <a href="{{url('')}}"><i class="fa fa-plus"></i>新增文章</a>
                <a href="#"><i class="fa fa-recycle"></i>批量删除</a>
                <a href="{{url('category/index')}}"><i class="fa fa-refresh"></i>文章分类</a>
            </div>
        </div>
    </div>
    <!--结果集标题与导航组件 结束-->
    ...
    <div class="result_wrap">
        <form action="{{url('admins/category/'.$field->cate_id)}}" method="post">
                    ...
                    <tr>
                        ...
                        <td>
                            <select name="cate_pid">
                                ...
                                <option
                                        @if($_data->cate_id==$field->cate_pid)
                                                selected="selected"
                                        @endif
                                        value="{{$_data->cate_id}}">
                                    <?php echo str_repeat('-',8*$_data['level']) ?>{{$_data->cate_name}}</option>
                                @endforeach
                            </select>
                        </td>
                    </tr>
